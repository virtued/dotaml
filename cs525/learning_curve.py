...
def plot_learning_curve(num_points, X_train, Y_train, X_test, Y_test, positive_class=1, negative_class=0):
    ...
    for training_set_size in training_set_sizes:
        ...
        print(accuracy)
        print('Accuracy for %d training examples: ', (training_set_size, accuracy))
...
def main():
    training_data = np.load('train_752845.npz')
    X_train = training_data['X']
    Y_train = training_data['Y']

    testing_data = np.load('test_83649.npz')
    X_test = testing_data['X']
    Y_test = testing_data['Y']

    #plot_learning_curve(30, X_train, Y_train, X_test, Y_test)
    plot_learning_curves(2, X_train, Y_train, X_test, Y_test)
